...
@Component({
  ...
  templateUrl: './homepage-editor-component-image-dialog.component.html',
  styleUrl: './homepage-editor-component-image-dialog.component.scss',
})
export class HomepageEditorComponentImageDialogComponent implements OnInit {
  public data: { component: ComponentInstance; languages: string[] } =
    inject(MAT_DIALOG_DATA);
  private dialogRef = inject(
    MatDialogRef<HomepageEditorComponentImageDialogComponent>,
  );
  ...
  initFormFromValues(): void {
    ...
    if (this.localizable) {
      ...
      valueGroup.addControl(
        'alt',
        new FormControl(this.data.component.value.alt ?? ''),
      );
      valueGroup.addControl(
        'title',
        new FormControl(this.data.component.value.title ?? ''),
      );
      this.form.addControl('value', valueGroup);
    }
  }
