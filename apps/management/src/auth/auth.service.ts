...
export class AuthService {
  ...
  async validateSSO(
    ...
    let user = await this.databaseService.userRepository.findOneBy({ provider, providerId });
    if (!user) {
      user = this.databaseService.userRepository.create({ provider, providerId, email, name });
  ...
  async createSession(user: UserEntity): Promise<SessionEntity> {
    ...
    const session = this.databaseService.sessionRepository.create({ user, expiresAt });
